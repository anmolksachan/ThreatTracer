...
BANNER = r"""[bold cyan]
 ...
 |__   __| |                  | |__   __|                     
    ...
    | |  | | | | | |  __/ (_| | |_ | | | | (_| | (_|  __/ |   
    |_|  |_| |_|_|  \___|\__,_|\__||_|_|  \__,_|\___\___|_|[/bold cyan][dim]  v5.0[/dim]
[italic dim]   CVE intelligence · risk triage · local-AI briefings — from a name, CPE, CVE, or URL[/italic dim]
...
def print_triage(summary: ScanSummary, top_n: int = 10) -> None:
    """Print the deterministic prioritised-findings table."""
    if not summary.top_findings:
        return
    t = Table(
        title="[bold]🎯 Prioritised Findings[/bold]  [dim](risk-ranked triage)[/dim]",
        box=box.ROUNDED,
        show_lines=True,
        expand=True,
    )
    ...
    t.add_column("CVE ID", style="bold", no_wrap=True, width=18)
    t.add_column("Prio", width=5, justify="center")
    t.add_column("Sev", width=4, justify="center")
    t.add_column("CVSS", width=5, justify="center")
    t.add_column("EPSS", width=6, justify="center")
    t.add_column("Maturity", width=11)
    t.add_column("Why it ranks", ratio=1)

    for i, item in enumerate(summary.top_findings[:top_n], 1):
        sev_style = _SEV_STYLE.get(item.severity, "")
        prio_style = ("bold red" if item.priority_score >= 75
                      else "red" if item.priority_score >= 50
                      else "yellow" if item.priority_score >= 25 else "dim")
        ...
        mat_style = _MATURITY_STYLE.get(item.exploit_maturity, "dim")
        comp = f"  [dim]({item.component})[/dim]" if item.component else ""
        t.add_row(
            str(i),
            f"[{sev_style}]{item.cve_id}[/{sev_style}]",
            f"[{prio_style}]{item.priority_score:.0f}[/{prio_style}]",
            f"[{sev_style}]{_SEV_EMOJI.get(item.severity, '')}[/{sev_style}]",
            cvss,
            epss,
            f"[{mat_style}]{item.exploit_maturity}[/{mat_style}]",
            item.reason + comp,
        )
    console.print(t)


def print_llm_summary(llm: LLMSummary) -> None:
    """Print the natural-language briefing in a panel."""
    if not llm or not llm.text:
        return
    if llm.engine in ("ollama", "openai"):
        title = f"[bold]🧠 AI Triage Briefing[/bold]  [dim]({llm.engine} · {llm.model})[/dim]"
        border = "magenta"
    else:
        title = "[bold]🧠 Triage Briefing[/bold]  [dim](heuristic engine)[/dim]"
        border = "cyan"
    body = llm.text
    if llm.degraded and llm.note:
        body += f"\n\n[dim]⚠ local LLM not used: {llm.note}[/dim]"
    console.print(Panel(body, title=title, border_style=border, expand=False, box=box.ROUNDED))
