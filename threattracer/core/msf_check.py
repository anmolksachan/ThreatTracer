...
class MSFCheckClient:
    ...
    async def _ensure_index(self) -> None:
        ...
        if not isinstance(data, dict):
            log.warning("Unexpected MSF metadata shape: %s", type(data).__name__)
            self._index = {}
            return
        for mod_path, mod in data.items():
            ...
            for ref in refs:
                ...
                if isinstance(ref, list) and len(ref) == 2 and ref[0] == "CVE":
                    # ref[1] is usually "2021-44228" but may already be prefixed
                    num = str(ref[1]).replace("CVE-", "").replace("cve-", "").strip()
                    cve_key = f"cve-{num}".lower()
