...
class GitHubPoCClient:
    ...
    async def find_pocs(self, cve_id: str) -> List[PoCReference]:
        ...
        """
        from threattracer.utils.validate import is_valid_cve
    ...
    async def _fetch_trickest(self, cve_id: str) -> List[PoCReference]:
        ...
        """
        from threattracer.utils.validate import cve_year
        year = cve_year(cve_id)
        if not year:
            log.debug("Skipping trickest for malformed CVE id: %r", cve_id)
            return []
