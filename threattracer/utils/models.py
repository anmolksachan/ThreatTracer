...
# ── Triage & summary models ────────────────────────────────────────────────

class TriageItem(BaseModel):
    """
    A single prioritised finding produced by the deterministic triage engine.

    ``priority_score`` is a 0-100 blend of KEV status, EPSS, CVSS, and the
    availability/maturity of public exploit tooling.  It is *decision support*,
    not a guarantee — it tells an operator what to look at first.
    """
    cve_id: str
    ...
    exploit_maturity: str = "none"      # none | poc | edb | weaponised
    signals: List[str] = Field(default_factory=list)
    reason: str = ""
    component: Optional[str] = None      # which detected tech surfaced it (asset scans)
    nvd_link: str = ""


class ScanSummary(BaseModel):
    """Structured, source-agnostic roll-up of a scan (component / cpe / cve / asset)."""
    target: str = ""
    scan_type: str = "component"        # component | cpe | cve | asset
    total_cves: int = 0
    critical: int = 0
    high: int = 0
    medium: int = 0
    ...
    with_msf: int = 0
    weaponised: int = 0
    top_findings: List[TriageItem] = Field(default_factory=list)


class LLMSummary(BaseModel):
    """Wraps the natural-language briefing plus how it was produced."""
    text: str = ""
    engine: str = "heuristic"          # heuristic | ollama | openai
    model: Optional[str] = None
    degraded: bool = False             # True if we wanted an LLM but fell back
    note: Optional[str] = None         # human-readable reason for degradation
