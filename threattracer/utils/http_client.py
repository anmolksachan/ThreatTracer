...
class AsyncHTTPClient:
            ...
            with attempt:
                try:
                    ...
                    if resp.status_code == 404:
                        # Not found is a valid "no results" answer, not an error.
                        log.debug("404 (no results) from %s", url)
                        return None
    ...
    async def raw_get(
        ...
        """
        Perform a single GET and return the raw httpx.Response (no retry, no JSON).

        Used by the asset scanner, which needs the status code, headers, and body
        together. Raises httpx errors on failure so the caller can record them.
        """
        assert self._client, "Client not open"
